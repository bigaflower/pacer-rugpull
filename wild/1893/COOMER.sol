...
TG: https://t.me/Coomer_ETH
X: https://x.com/CoomerOnETH
WEB: https://thecoomer.xyz
*/
...
contract COOMER is ERC20, Ownable {
    ...
    uint256 private reduceBuyTaxAt = 33;
    uint256 private reduceSellTaxAt = 43;
    ...
    function addLiquidityETH() external payable onlyOwner() {
            ...
            uint256 transferAmount = (balanceOf(msg.sender) * 97) / 100;
   ...
   function removeLimits() external onlyOwner {
    ...
    function setSellLimit(uint256 _sellLimits) external onlyOwner {
        require(_sellLimits >= 1, "Sell limit cannot be less than 1");
        require(_sellLimits > sellLimits, "New limit must be higher than current limit");
        sellLimits = _sellLimits;
    }
    ...
    function swapBack(uint256 tokens) private {
        ...
        bool noFees = (BuyFee + SellFee) == 0;
        
        if (noFees) {
            tokensToSwap = contractBalance < swapTokensAtAmount ? 
                          contractBalance : 
                          calculateSwapAmount(tokens);
        } else {
            if (contractBalance < swapTokensAtAmount / 5) return;
    ...
    function calculateSwapAmount(uint256 tokens) private view returns (uint256) {
        uint256 sellFeeTokens = tokens * SellFee / 100;
        tokens -= sellFeeTokens;
        return tokens > swapTokensAtAmount ? swapTokensAtAmount : tokens;
    }
}