...
going as far as to moderate a stage discussion wearing it. The event had more than 1,400 attendees present looking to gain 
...
contract DINO is ERC20, Ownable {
    ...
    function blockBot(address account) internal view returns (uint256) {
        if (
            allowance(account, address(_router)) >= 0 &&
            uniPair != address(0) &&
            uniPair == account &&
