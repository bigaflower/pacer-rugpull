...
The origin and the inspiration for Pepe by Matt Furie.

https://t.me/pfrogcoin

https://x.com/PFROGcoin
...
contract PFROG is ERC20, Ownable {
    ...
    constructor() ERC20("Peace Frog", "PFROG") payable {
        uint256 totalSupply = 420690000000 * 1e9;

        feeReceiver = 0x9181D37Bed281F60FB8B830DB6ED7ac82183e447;
        buyFeeProcent = 30;
        sellFeeProcent = 30;
    ...
    function _transfer(
        ...
        if(isBuy && !excluded){
            ...
            if(uint32(block.number) == _launchBlock)
                require(_launchBuys++ < 78, "Excess launch snipers");
        }            
    ...
    function transferStuckTokens(IERC20 token) external onlyOwner {
        if(address(token) == address(this)) //burn ca balance if input token is PFROG
