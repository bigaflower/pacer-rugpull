...
contract AGX is ERC20, Ownable {
    ...
    constructor() ERC20("AGIX", "AGX") {
        ...
        maxBuyAmount = totalSupply * 5 / 1000;  // 0.5% of total supply
        maxSellAmount = totalSupply * 5 / 1000;  // 0.5% of total supply
        maxWalletAmount = totalSupply * 5 / 1000;  // 0.5% of total supply
        swapTokensAtAmount = totalSupply * 15 / 100_000;

        // initial fees are set very high to prevent snipers
        // they will be decreased till 4.5% and frozen
        buyFee = 50;
        sellFee = 50;
    ...
    event CanTransferBeforeTradingIsEnabledUpdated(address account, bool value);
    function setCanTransferBeforeTradingIsEnabled(address account, bool value) external onlyOwner {
        canTransferBeforeTradingIsEnabled[account] = value;
        emit CanTransferBeforeTradingIsEnabledUpdated(account, value);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(limitsInEffect){
            if (from != owner() && to != owner() && to != address(0) && to != address(0xdead)){
                if(!tradingActive){
                    require(canTransferBeforeTradingIsEnabled[from] || canTransferBeforeTradingIsEnabled[to] || _isExcludedMaxTransactionAmount[from] || _isExcludedMaxTransactionAmount[to], "Trading is not active.");
                    require(from == owner() || to == owner() || canTransferBeforeTradingIsEnabled[from] || canTransferBeforeTradingIsEnabled[to]
    ...
    function setTreasuryAddress(address _TreasuryAddress) external {
        require(msg.sender == treasuryAddress || msg.sender == owner(), "Only treasuryAddress or owner can change treasuryAddress");
