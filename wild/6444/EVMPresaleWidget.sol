...
contract SecuryWalletPresale is OwnerWithdrawable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    modifier saleEnabled() {
        require(saleStatus == true, "EVMWidget is not enabled");
        _;
    }

    modifier saleStoped() {
        require(saleStatus == false, "EVMWidget is not stopped");
        _;
    }
    ...
    function addPayableTokens(
        ...
        require(
            ...
            "EVMWidget tokens & prices arrays length mismatch"
        );
    ...
    function getTokenAmount(
        ...
        uint256 amtOut;
        if (token != address(0)) {
            require(payableTokens[token] == true, "EVMWidget Token not allowed");
    ...
    function buyToken(
        ...
        require(saleTokenAmt != 0, "EVMWidget Amount is 0");
        require(
            (totalTokensSold + saleTokenAmt) < totalTokensforSale,
            "EVMWidget Not enough tokens to be sale"
        );
    ...
    function listBuyers(
        ...
        require(_from < _to, "EVMWidget _from should be less than _to");
