 ...
 * https://wen-lambo.financial/
 * https://t.me/wenlambofi
 * https://x.com/wenlambofi
 */
...
contract Lambo is ERC20, Ownable {
    ...
    constructor() ERC20("Wen Lambo Financial", "WLFI") {
        IDexRouter _uniRouter = IDexRouter(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        );

        setAddressLimitExempt(address(_uniRouter), true);
        ...
        setAddressLimitExempt(address(uniPair), true);
        _toggleDexPair(address(uniPair), true);

        uint256 _totalSupply = 1_000 * 10 ** decimals();
        ...
        marketingWallet = address(0xDD2Ed6ea344c555E7C35C8cCa2B43e42Ce0e0241);

        // exclude from paying fees or having max transaction amount
        setAddressTaxExempt(msg.sender, true);
        setAddressTaxExempt(address(this), true);
        setAddressTaxExempt(address(0xdead), true);
        setAddressTaxExempt(marketingWallet, true);

        setAddressLimitExempt(msg.sender, true);
        setAddressLimitExempt(address(this), true);
        setAddressLimitExempt(address(0xdead), true);
        setAddressLimitExempt(marketingWallet, true);
     ...
     */
    function setMaxLimitTx(uint256 _maxTx) external onlyOwner {
     ...
     */
    function setAddressLimitExempt(
        address _add,
        bool _excluded
    ) public onlyOwner {
     ...
     */
    function taxesSetBuy(uint256 _value) external onlyOwner {
        buyTaxTotal = _value;
     ...
     * @param _value The fee for the marketing wallet
     */
    function taxesSetSell(uint256 _value) external onlyOwner {
        sellTaxTotal = _value;
        require(
            sellTaxTotal <= 100,
    ...
    function taxesSetTransfer(uint256 _value) external onlyOwner {
