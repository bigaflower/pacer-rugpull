...
// File contracts/KiyositoToken.sol
...
/**
 * @title Kiyosito
 * @dev ERC20 token with pausing, access control, gasless approvals, and robustness improvements.
 */
contract Kiyosito is
    ERC20,
    ERC20Permit,
    ERC20Burnable,
    Pausable,
    ...
    ReentrancyGuard
{
    // Keep SafeERC20 usage
    using SafeERC20 for IERC20;

    // Define roles
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant CROWDSALE_OPERATOR_ROLE =
        keccak256("CROWDSALE_OPERATOR_ROLE");

    // Custom errors for gas optimization
    error InsufficientBalance(address account, uint256 balance, uint256 amount);
    error ZeroAddressNotAllowed();

    // Events for role revocation, pausing, and token recovery
    event RoleRevokedWithCheck(
     ...
     * Grants the deployer the default admin and pauser roles.
     */
    constructor() ERC20("Kiyosito", "ITO") ERC20Permit("Kiyosito") {
        require(msg.sender != address(0), "Zero address not allowed");

        // Mint 100,000,000 tokens with 18 decimals to the deployer
    ...
    /**
     * @dev Explicitly set decimals to 18 for clarity.
     */
    ...
    /**
     * @dev Pause token transfers. Can only be called by an account with the PAUSER_ROLE.
     */
    ...
    /**
     * @dev Unpause token transfers. Can only be called by an account with the PAUSER_ROLE.
     */
    ...
    /**
     * @dev Overrides the internal _update function to enforce pause state and custom transfer logic.
     * Ensures that token transfers are blocked when paused.
     */
    function _update(
        address from,
        address to,
        ...
        if (paused()) {
            require(
                hasRole(CROWDSALE_OPERATOR_ROLE, _msgSender()) ||
                    hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
                "Transfers paused during crowdsale"
            );
        }
        super._update(from, to, amount); // Pausable's check happens automatically
    }
    ...
    function recoverERC20(
        ...
        emit TokensRecovered(tokenAddress, tokenAmount, msg.sender);
    }
    ...
    /**
     * @dev Revokes a role with checks and emits an event to ensure operational safety.
    ...
    function revokeRoleWithCheck(
        ...
        emit RoleRevokedWithCheck(role, account, msg.sender);
    }

    /**
     * @dev Restrict token burning to admin and crowdsale operator (e.g., ICO contract).
     */
