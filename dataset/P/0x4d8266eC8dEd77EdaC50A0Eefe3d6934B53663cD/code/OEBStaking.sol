...
contract OEBStaking is Ownable, ReentrancyGuard {
    ...
    uint256 public constant DECIMALS_HELPER = 1e15;
    ...
    constructor(
        ...
        pool.lpToken = IERC20(_ofi);
        pool.lastRewardBlock = startBlock;
        pool.accRewardPerShare = 0;
        pool.currentDepositAmount = 0;

        remainingRewards = _totalReward;
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            if (remainingRewards > 0) {
                ...
                uint256 totalRewards = multiplier.mul(rewardsPerBlock);
                totalRewards = Math.min(totalRewards, remainingRewards);
                accRewardPerShare = accRewardPerShare.add(totalRewards.mul(DECIMALS_HELPER).div(lpSupply));
            }
        }
        return user.amount.mul(accRewardPerShare).div(DECIMALS_HELPER).sub(user.rewardDebt);
    }
    ...
    function updatePool() public {
        ...
        pool.accRewardPerShare = pool.accRewardPerShare.add(totalRewards.mul(DECIMALS_HELPER).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        updatePool();
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accRewardPerShare).div(DECIMALS_HELPER).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accRewardPerShare).div(DECIMALS_HELPER);
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        bool transferSuccess = rewardToken.transfer(_to, _amount);
