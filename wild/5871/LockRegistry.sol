...
abstract contract LockRegistry is Ownable {
    ...
    // When true, token owner cannot unlock
    bool public tokenOwnerUnlockDisabled = false;
    ...
    function setTokenOwnerUnlockDisabled(bool disabled) external onlyOwner {
        tokenOwnerUnlockDisabled = disabled;
    }
    ...
    function _lockId(uint256 _id) internal {
        require(
            approvedContract[msg.sender] || msg.sender == _ownerOf(_id),
            "Cannot update map"
        );
        require(
    ...
    function _unlockId(uint256 _id) internal {
        require(
            approvedContract[msg.sender] ||
                (!tokenOwnerUnlockDisabled && msg.sender == _ownerOf(_id)),
            "Cannot update map"
        );
    ...
    function _freeId(uint256 _id, address _contract) internal {
        // Only approved contracts can free IDs
