...
contract UTC is IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(pairs[sender] || pairs[recipient]) {
            uint256 burnAmount = amount * 3 / 100; // 3% DApp费用
            _transferNormal(sender, burnAddress, burnAmount); // 转账3% DApp费用到 dappAddress
            receiveAmount -= burnAmount; // 更新接收方应接收的金额
        }
    ...
    function setBurnAddress(address newBurnAddress) public onlyOwner {
        require(newBurnAddress != address(0), "New dapp address cannot be the zero address");
