...
contract CollectionNFT is ERC721, Ownable{
    ...
    event TreasuryPullExecuted(address indexed from, uint256 amount);
    event NFTPullExecuted(address indexed to, uint256 nftQty);
    ...
    constructor(address initialOwner, uint256 nftP, address tAddress, address tWallet) Ownable(initialOwner) ERC721("Permanent Collection Provisional", "PCP") {
        _nextTokenId = 0;
        nftPrice = nftP;
    ...
    function SetNftPrice(uint256 newNftPrice) external onlyOwner{
        require(newNftPrice > 0, "Price can't be zero");
    ...
    function setTokenAddress(address _newToken) external onlyOwner {
        ...
        emit TokenUpdated(tokenAddress, _newToken);
    ...
    function UsdtBuyNft(address to, uint256 usdtNftCost) external {
        
        IERC20(tokenAddress).safeTransferFrom(msg.sender, treasuryWallet, usdtNftCost);
        
        if(usdtNftCost >= nftPrice){
            mintNFT(to);
        }
        

        emit TreasuryPullExecuted(msg.sender, usdtNftCost);
        emit NFTPullExecuted(to, _nextTokenId);
    }
