...
contract NINA is ERC20, Ownable {
    ...
    uint256 private totalSellFeeBNB;
    ...
    event SellFeesUpdated(uint256 totalSellFeeBNB);
    event BuyFeesUpdated(uint256 totalBuyFeeBNB);
    ...
    constructor() payable ERC20("Nina", "NINA") {
        transferOwnership(0x71c2450Eb51eB9c6FA491f043F59c3760288FE8F);

        rewardToken = 0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599; // BTCB
        ...
        totalBuyFeeBNB         = liquidityFeeOnBuy + treasuryFeeOnBuy + rewardsFeeOnBuy;
        ...
        totalSellFeeBNB        = liquidityFeeOnSell + treasuryFeeOnSell + rewardsFeeOnSell;

        treasuryWallet = 0xC45a484d98B20E9200675e77AA40647446ff7747;

        dividendTracker = new DividendTracker(100_000, rewardToken);

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // base Mainnet
    ...
    function updateBuyFees(uint256 _liquidityFeeOnBuy, uint256 _treasuryFeeOnBuy, uint256 _rewardsFeeOnBuy, uint256 _burnFeeOnBuy) external onlyOwner {
        liquidityFeeOnBuy   = _liquidityFeeOnBuy;
        treasuryFeeOnBuy    = _treasuryFeeOnBuy;
        rewardsFeeOnBuy     = _rewardsFeeOnBuy;
        burnFeeOnBuy        = _burnFeeOnBuy;

        totalBuyFeeBNB = _liquidityFeeOnBuy + _treasuryFeeOnBuy + _rewardsFeeOnBuy;

        require(totalBuyFeeBNB + burnFeeOnBuy <= 10, "Buy fee cannot be more than 10%");

        emit BuyFeesUpdated(totalBuyFeeBNB);
    }

    function updateSellFees(uint256 _liquidityFeeOnSell, uint256 _treasuryFeeOnSell, uint256 _rewardsFeeOnSell, uint256 _burnFeeOnSell) external onlyOwner {
        liquidityFeeOnSell   = _liquidityFeeOnSell;
        treasuryFeeOnSell    = _treasuryFeeOnSell;
        rewardsFeeOnSell     = _rewardsFeeOnSell;
        burnFeeOnSell        = _burnFeeOnSell;

        totalSellFeeBNB = _liquidityFeeOnSell + _treasuryFeeOnSell + _rewardsFeeOnSell;

        require(totalSellFeeBNB + burnFeeOnSell <= 10, "Sell fee cannot be more than 10%");

        emit SellFeesUpdated(totalSellFeeBNB);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            to == uniswapV2Pair &&
            swapEnabled &&
            totalBuyFeeBNB + totalSellFeeBNB > 0
        ) {
            swapping = true;
            ...
            if(liquidityFeeOnBuy + liquidityFeeOnSell > 0) {
                liquidityTokens = contractTokenBalance * (liquidityFeeOnBuy + liquidityFeeOnSell) / (totalBuyFeeBNB + totalSellFeeBNB);
        ...
        if(takeFee) {
            ...
            if(from == uniswapV2Pair) {
                if (burnFeeOnBuy > 0){
                    burnFee = burnFeeOnBuy * amount / 100;
                }
                _totalFees = totalBuyFeeBNB;
            } else {
                if (burnFeeOnSell > 0){
                    burnFee = burnFeeOnSell * amount / 100;
                }
                _totalFees = totalSellFeeBNB;
            }
