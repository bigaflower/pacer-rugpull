...
contract DuckyCityIsland is ERC721A, Ownable, Pausable, PaymentSplitter {
  ...
  mapping(address => uint256) public _purchases;
  mapping(address => uint256) public _presaleMaxPerAddressMap;
  ...
  function setPresaleMaxPerAddress(
    address[] memory whitelist,
    uint256[] memory maxPerAddress
  ) external onlyOwner {
    require(whitelist.length == maxPerAddress.length, "whitelist and maxPerAddress length mismatch");
    require(whitelist.length > 0, "No whitelist");

    for (uint256 i = 0; i < whitelist.length; i++) {
      _presaleMaxPerAddressMap[whitelist[i]] = maxPerAddress[i];
    }
  }
  ...
  function earlyPurchase(uint256 count, bytes32[] calldata merkleProof) external payable whenNotPaused {
    ensurePublicMintConditions(msg.sender, count, _presaleMaxPerAddressMap[msg.sender]);
