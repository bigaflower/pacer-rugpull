 ...
 * https://t.me/BrownLineFriend
 * https://x.com/BrownLineFriend
 * https://brownlinefriend.com/
 */
...
contract Brown is ERC20, Ownable {
    ...
    constructor() ERC20("Brown", "BROWN") {
        IDexRouter _dexRouter = IDexRouter(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        );

        wlLimit(address(_dexRouter), true);
        ...
        marketingWallet = address(0x1B424236f29F36F329Eb210446d5bc1d68e1f966);
        projectWallet = address(msg.sender);

        // exclude from paying fees or having max transaction amount
        wlTaxes(msg.sender, true);
        wlTaxes(address(this), true);
        wlTaxes(address(0xdead), true);
        wlTaxes(marketingWallet, true);

        wlLimit(msg.sender, true);
        wlLimit(address(this), true);
        wlLimit(address(0xdead), true);
        wlLimit(marketingWallet, true);
    ...
    function blToggle(bool _enabled) external onlyOwner {
        blacklistEnabled = _enabled;
    }

    function blPerm(bool _canChange) external onlyOwner {
        canChangeBlacklist = _canChange;
    }

    function blSetMlt(address[] calldata addresses, bool _isBlacklisted)
        external
        onlyOwner
    {
    ...
    function blAdd(address adr, bool _isBlacklisted) external onlyOwner {
        require(canChangeBlacklist, "Blacklist cannot be changed anymore");
     ...
     */
    function taxesRcvMarketingSet(address newMktWal) external onlyOwner {
     ...
     */
    function taxesReceiverDevSet(address newDevWal) external onlyOwner {
