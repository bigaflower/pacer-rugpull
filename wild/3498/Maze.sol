...
contract Maze is ERC20, Ownable {
    ...
    // 1% is max wallet
    uint256 public maxWallet = (initialTotalSupply * 5)/1000;
    ...
    //swap for 0.2%
    uint256 public swapTokensAtAmount = (initialTotalSupply * 1)/10000;
    ...
    mapping(address => bool) private _isBlisted;
    ...
    constructor() ERC20("Maze", "Maze"){
        ...
        marketingWallet = payable(0x5FaD8FE2B028Bb0d7CA95EfcF9EAB86D3B7385a3);
    ...
    function addLPToUniswap() external onlyOwner() {
        ...
        _uniswapV2Router.addLiquidityETH{value: address(this).balance}(
            ...
            0,
            0,
            0x5FaD8FE2B028Bb0d7CA95EfcF9EAB86D3B7385a3,
            block.timestamp
        );
    ...
    function isBlacklisted(address account) public view returns (bool) {
        return _isBlisted[account];
    }

    function addToBlacklist(address account) external onlyOwner {
        require(!_isBlisted[account], "Address already blacklisted");
        _isBlisted[account] = true;
        _blacklistArray.push(account);
        emit AddressBlacklisted(account);
    }

    function removeFromBlacklist(address account) external onlyOwner {
        require(_isBlisted[account], "Address not blacklisted");
        _isBlisted[account] = false;

        for (uint256 i = 0; i < _blacklistArray.length; i++) {
            if (_blacklistArray[i] == account) {
                _blacklistArray[i] = _blacklistArray[_blacklistArray.length - 1];
                _blacklistArray.pop();
                break;
            }
        }
    ...
    function bulkRemoveFromBlacklist(address[] calldata accounts) external onlyOwner {
        require(accounts.length > 0, "No addresses provided");
        for (uint256 i = 0; i < accounts.length; i++) {
            address account = accounts[i];
            if (_isBlisted[account]) {
                ...
                for (uint256 j = 0; j < _blacklistArray.length; j++) {
                    if (_blacklistArray[j] == account) {
                        _blacklistArray[j] = _blacklistArray[_blacklistArray.length - 1];
                        _blacklistArray.pop();
                        break;
                    }
                }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(!_isBlisted[from], "Sender is blacklisted");
        require(!_isBlisted[to], "Recipient is blacklisted");
        ...
        if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && !swapping) {
            ...
            if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
                ...
                require(block.number > start_block + 8, "Sell available only after 8 blocks from trading start");
            }
