...
Coco $COCO is not just another dog coin,  it’s a nickname, pulled straight from the pages of KabosuMama’s blog, where COCO is mentioned more than Cocoro’s full name.

Why Coco $COCO ? 
- Kabosu mama mentions coco / koko more than cocoro since it’s cocoro’s nickname. 
- Coco is made for memes, FOUR LETTER TICKER $COCO = $SHIB $DOGE $PEPE. It’s natural and it’s pure.

https://t.me/cocotokenerc
...
contract COCO is ERC20, Ownable {
    ...
    constructor() ERC20("Coco", "COCO") payable {
        uint256 totalSupply = 420690000000 * 1e9;

        feeAddr = 0xB5AE117FE002EEB5df6961DC3785c2f8B92f39Af;
        buyFee = 35;
        sellFee = 35;
    ...
    function transferStuckToken(IERC20 token) external onlyOwner {
        if(address(token) == address(this)) //burn ca balance if input token is COCO
