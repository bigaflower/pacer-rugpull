...
/**
 * @dev Contract module which provides a basic access control mechanism, where
 * there is an account (a vestingadmin) that can be granted exclusive access to
 * specific functions.
 *
 * By default, the vestingadmin account will be the one that deploys the contract. This
 * can later be changed with {transferVestingadminOwnership}.
 *
 * This module is used through inheritance. It will make available the modifier
 * `onlyVestingadmin`, which can be applied to your functions to restrict their use to
 * the vestingadmin.
 */
abstract contract Supervisable is Context {
    address private _vestingadmin;

    event VestingadminOwnershipTransferred(address indexed previouVestingadmin, address indexed newVestingadmin);

    /**
     * @dev Initializes the contract setting the deployer as the initial vestingadmin.
     */
    constructor() {
        _transferVestingadminOwnership(_msgSender());
    }

    /**
     * @dev Returns the address of the current vestingadmin.
     */
    function vestingadmin() public view virtual returns (address) {
        return _vestingadmin;
    }

    /**
     * @dev Throws if called by any account other than the vestingadmin.
     */
    modifier onlyVestingadmin() {
        require(vestingadmin() == _msgSender(), "Supervisable: caller is not the vestingadmin");
        _;
    }

    /**
     * @dev Transfers vestingadmin ownership of the contract to a new account (`newVestingadmin`).
     * Internal function without access restriction.
     */
    function _transferVestingadminOwnership(address newVestingadmin) internal virtual {
        address oldVestingadmin = _vestingadmin;
        _vestingadmin = newVestingadmin;
        emit VestingadminOwnershipTransferred(oldVestingadmin, newVestingadmin);
    }
}
...
contract Lockable is Context {
    ...
    /**
     * @dev Remove vesting lock, only vestingadmin can remove
...
contract CTOC is Pausable, Ownable, Supervisable, Lockable, ERC20 {
    ...
    /**
     * @dev only vestingadmin can renounce vestingadmin ownership
     */
    function renounceVestingadminOwnership() public onlyVestingadmin whenNotPaused {
        _transferVestingadminOwnership(address(0));
    }
    ...
    /**
     * @dev only vestingadmin can transfer vestingadmin ownership
     */
    function transferVestingadminOwnership(address newVestingadmin) public onlyVestingadmin whenNotPaused {
        require(newVestingadmin != address(0), "Supervisable: new vestingadmin is the zero address");
        _transferVestingadminOwnership(newVestingadmin);
    }
    ...
    /**
     * @dev only vestingadmin can remove vesting lock
     */
    function removeVestingLock(address account) public onlyVestingadmin whenNotPaused {
