...
// Flattened for Remix - do not edit. Generated from QuinesteinsCymaticStudio.sol
...
// --- node_modules\@openzeppelin\contracts\token\ERC721\IERC721.sol ---
/**
...
// --- node_modules\@openzeppelin\contracts\token\ERC721\extensions\IERC721Metadata.sol ---
/**
...
// --- node_modules\@openzeppelin\contracts\token\ERC721\IERC721Receiver.sol ---
/**
...
// --- node_modules\@openzeppelin\contracts\token\ERC721\utils\ERC721Utils.sol ---
/**
...
library Bytes {
    ...
    function slice(bytes memory buffer, uint256 start, uint256 end) internal pure returns (bytes memory) {
        ...
        // allocate and copy (no mcopy: use loop for pre-Cancun EVM)
        bytes memory result = new bytes(end - start);
        uint256 len = end - start;
        assembly {
            let dest := add(result, 0x20)
            let src := add(add(buffer, 0x20), start)
            let fullLen := and(len, not(31))
            for { let i := 0 } lt(i, fullLen) { i := add(i, 32) } {
                mstore(add(dest, i), mload(add(src, i)))
            }
            let tail := and(len, 31)
            if tail {
                let lastStart := sub(len, tail)
                let word := mload(add(src, lastStart))
                let mask := sub(shl(mul(tail, 8), 1), 1)
                mstore(add(dest, lastStart), and(word, mask))
            }
        }
    ...
    function splice(bytes memory buffer, uint256 start, uint256 end) internal pure returns (bytes memory) {
        ...
        // move and resize (no mcopy: use loop for pre-Cancun EVM)
        uint256 len = end - start;
        assembly {
            let dest := add(buffer, 0x20)
            let src := add(add(buffer, 0x20), start)
            let fullLen := and(len, not(31))
            for { let i := 0 } lt(i, fullLen) { i := add(i, 32) } {
                mstore(add(dest, i), mload(add(src, i)))
    ...
    function replace(
        ...
        // replace (no mcopy: use loop for pre-Cancun EVM)
        assembly {
            let dest := add(add(buffer, 0x20), pos)
    ...
    function concat(bytes[] memory buffers) internal pure returns (bytes memory) {
        ...
        for (uint256 i = 0; i < buffers.length; ++i) {
            ...
            assembly {
                let dest := add(result, offset)
                let src := add(input, 0x20)
                let fullLen := and(len, not(31))
                for { let j := 0 } lt(j, fullLen) { j := add(j, 32) } {
...
// --- node_modules\@openzeppelin\contracts\token\ERC721\extensions\IERC721Enumerable.sol ---
/**
...
// --- node_modules\@openzeppelin\contracts\token\ERC721\extensions\ERC721Enumerable.sol ---
/**
...
// --- contracts\QuinesteinsCymaticStudio.sol ---
/**
 * Quinesteins Cymatic Studio – standalone ERC721 collection
 * - Stores one metadata URI per token in _tokenURIs[tokenId]. App sets each token's URI
 *   via setTokenURI/setTokenURIs when metadata is created (after mint). Each owner's tokens
 *   have their URIs stored individually; tokenURI(tokenId) returns the stored URI.
 * - Ownership is updated automatically on transfer/sale by ERC721 (transferFrom, safeTransferFrom).
 * - Supply: totalSupply() / totalMinted(). Minting: mint(uint256[] tokenIds) payable.
 */
contract CymaticStudio is ERC721Enumerable, Ownable, ReentrancyGuard {
    uint256 public immutable PRICE_PER_TOKEN_ETH;
    ...
    constructor() ERC721("Cymatic Studio", "CYMATICS") Ownable(msg.sender) ReentrancyGuard() {
        PRICE_PER_TOKEN_ETH = 0.003 ether;  // 0.003 ETH per token (matches Cymatics UI)
        MAX_SUPPLY = 10000;
        _baseTokenUri = "";
        // Owner mints 1–20 via UI (owner mint button). Public mint starts at token ID 21.
    }
    ...
    // --- Minting ---
    // Owner can mint for free; others pay PRICE_PER_TOKEN_ETH per token.

    function mint(uint256[] calldata tokenIds) external payable nonReentrant {
        uint256 count = tokenIds.length;
        require(count > 0, "No token IDs");
        require(totalSupply() + count <= MAX_SUPPLY, "Exceeds max supply");

        uint256 required = msg.sender == owner() ? 0 : (PRICE_PER_TOKEN_ETH * count);
    ...
    // --- Metadata: store app-created URIs and return them from tokenURI ---
    ...
    function setTokenURI(uint256 tokenId, string calldata uri) external {
        ...
        if (tokenOwner != address(0)) {
            require(tokenOwner == msg.sender || owner() == msg.sender, "Not owner");
        }
