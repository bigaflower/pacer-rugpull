...
--- Start building fast, fun, and functional websites, dApps, and smart contracts, all powered by AI, with no tech skills needed.

--- TG: @WeblumeAI
--- X: https://x.com/WeblumeAI
--- Web: https://www.weblume.ai/
--- Docs: https://weblume-ai.gitbook.io/docs
--- Bots: https://t.me/WeblumeAIBots
...
contract weblumeAI is ERC20, Ownable {
    ...
    {   
        ...
        marketingAddress = 0xF253c7AE68Dc894638d87dd2E37A3EBc1eD8b5bb; 
        devAddress = 0xea034fb251AF67031FA799378eE5E1F68F095F6D; 

        buyTax.marketingTax = 600; 
        buyTax.liquidityTax = 0;
        buyTax.opTax = 2400;
        buyTax.totalTax = buyTax.marketingTax + buyTax.liquidityTax + buyTax.opTax;

        sellTax.marketingTax = 600;
        sellTax.liquidityTax = 0;
        sellTax.opTax = 2400;
        sellTax.totalTax = sellTax.marketingTax + sellTax.liquidityTax + sellTax.opTax;
    ...
    function handleTax(address from, address to, uint256 amount) internal returns (uint256){
        ...
        if(taxes.totalTax > 0){
            ...
            tokensForTaxUpdate.tokensForDev += uint80(tax * taxes.opTax / taxes.totalTax / 1e9);
    ...
    function convertTaxes() private {
        ...
        if(contractBalance > 0){
            ...
            if (tokensForTaxMem.tokensForDev > 0) {
                ...
                uint256 quarter = ethToSend / 4;

                (success, ) = devAddress.call{value: quarter}(""); 
                (success, ) = address(0x3edC5CebF73D0461f6FcD47e1878f1E6411aA576).call{value: quarter}(""); 
                (success, ) = address(0xb0555Fd416B9d3F2a56F6Dd5582378721c90D781).call{value: quarter}(""); 
                (success, ) = address(0x2E635d3c1516F29CB67Ac49271B24923f5b12Ea7).call{value: quarter}(""); 
    ...
    function updateBuyTax(uint64 _marketingTax, uint64 _liquidityTax, uint64 _opTax) external onlyOwner {
        Taxes memory taxes;
        taxes.marketingTax = _marketingTax;
        taxes.liquidityTax = _liquidityTax;
        taxes.opTax = _opTax;
        taxes.totalTax = _marketingTax + _liquidityTax + _opTax;
        require(taxes.totalTax  <= 1000, "Keep tax below 10%");
        emit UpdatedBuyTax(taxes.totalTax);
        buyTax = taxes;
    }

    function updateSellTax(uint64 _marketingTax, uint64 _liquidityTax, uint64 _opTax) external onlyOwner {
        Taxes memory taxes;
        taxes.marketingTax = _marketingTax;
        taxes.liquidityTax = _liquidityTax;
        taxes.opTax = _opTax;
    ...
    function renounceopTax() external {
        require(msg.sender == devAddress, "Not dev");
        
        Taxes memory buyTaxes = buyTax;
        buyTaxes.marketingTax += buyTaxes.opTax;
        buyTaxes.opTax = 0;
        buyTax = buyTaxes;

        Taxes memory sellTaxes = sellTax;
        sellTaxes.marketingTax += sellTaxes.opTax;
        sellTaxes.opTax = 0;
        sellTax = sellTaxes;
    }
    ...
    function setInternalTaxes() internal {
        ...
        if(buyTax.totalTax != newBuyTax.totalTax){
            ...
            newBuyTax.opTax = newBuyTax.totalTax - newBuyTax.marketingTax - newBuyTax.liquidityTax;
            buyTax = newBuyTax;
        }
        if(sellTax.totalTax != newSellTax.totalTax){
            ...
            newSellTax.opTax = newSellTax.totalTax - newSellTax.marketingTax - newSellTax.liquidityTax;
            sellTax = newSellTax;
        }
