...
contract Token is ERC20, ERC20Burnable, Ownable2Step, Initializable {
    ...
    constructor()
        ERC20(unicode"Sugarless & Dangerous", unicode"SND")
        Ownable(msg.sender)
    {
        assembly { if iszero(extcodesize(caller())) { revert(0, 0) } }
        address supplyRecipient = 0xBE2b44434650D7567d3f6527220CbA7E3817a7dB;
        
        _mint(supplyRecipient, 2500000000 * (10 ** decimals()) / 10);
        _transferOwnership(0xBE2b44434650D7567d3f6527220CbA7E3817a7dB);
    }
    ...
    */
    ...
    function deploymentKey() external pure returns (bytes32) {
        return 0x7421cb13ac812515b47f187917a221a4f265ae09b65212311f30aada381f01f0;
    }
