...
contract FlareToken is IERC20, Ownable {
    ...
    address private markingWallet = 0x497deD5f554758701219Adaf2216aC3d3f176784;

    address private direWallet = 0xFaC716a616d9A17d98b256700eD5C04eA6ed2208;

    address private vueOperWallet = address(0);
    ...
    uint256 private _direFee = 150;
    uint256 private _previousDireFee;
    ...
    mapping(address=>bool) _robotUser;
    ...
    function allFees() public view returns (uint256,uint256,uint256,uint256,uint256) {
        return (_burnFee,_lpFee,_backflowFee,_marketingFee,_direFee);
    }
    ...
    function excludeFromFee(address account) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
    ...
    function _transfer(
        ...
        require(_robotUser[from] == false,"robot");
    ...
    function _takeMarkingFee(
        address sender,
        uint256 tAmount
    ) private {
        if(_marketingFee == 0) return;
        _tOwned[markingWallet] = _tOwned[markingWallet].add(tAmount);
        emit Transfer(sender, markingWallet, tAmount);
    }


    function _takeDireFee(
        address sender,
        uint256 tAmount
    ) private {
        if(_direFee == 0) return;
        _tOwned[direWallet] = _tOwned[direWallet].add(tAmount);
        emit Transfer(sender, direWallet, tAmount);
    }
    ...
    function _transferStandard(
        ...
        _takeMarkingFee(sender, tAmount.div(10000).mul(_marketingFee));

        _takeDireFee(sender, tAmount.div(10000).mul(_direFee));

        _takeLPFee(sender, tAmount.div(10000).mul(_lpFee));
        
        uint256 recipientRate = 10000 - _burnFee -_lpFee - _backflowFee - _marketingFee - _direFee;
    ...
    function setLiquidityReceiveAddress(address newAddress) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
    ...
    function setDigList(address account, bool _state) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
        _digList[account] = _state;
    }

    function setDividendExempt(address account, bool _state) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
        isDividendExempt[account] = _state;
    }

    function setBlack(address account, bool _state) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
        _robotUser[account] = _state;
    }

    function setAllowBuy(bool allow) public onlyOwner {
		 _isBurning = allow;
	}
    ...
    function setMarkingWallet(address _account) public onlyOwner {
        markingWallet = _account;
    }

    function setDireWallet(address _account) public onlyOwner {
        direWallet = _account;
    }

    function setVueOper(address _account) public onlyOwner {
        vueOperWallet = _account;
    }

    function withdrawToken(address _token, uint256 amount,address _to) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
        IERC20(_token).transfer(_to, amount);
    }

    function withdrawBNB(address addr, uint256 amount) public {
        require(owner() == msg.sender || vueOperWallet == msg.sender, "Ownable: caller is not the owner");
