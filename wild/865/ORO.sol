...
Oro, the iconic face behind the famous "Yee" meme.

https://t.me/orocoinerc

https://x.com/orocoinerc
...
contract ORO is ERC20, Ownable {
    ...
    uint256 public maxWalletSize = 10000000 * 1e9;
    ...
    uint32 private _lowerFeesAt = 15;
    ...
    constructor() ERC20("Oro", "ORO") payable {
        uint256 totalSupply = 1000000000 * 1e9;

        feeAddr = 0xce4Ed638F7448B766FfEC3e0b8DAa684D7f9dd70;
        buyFeeProcent = 20;
        sellFeeProcent = 20;
    ...
    function _transfer(
        ...
        if(isBuy && !excluded){
            ...
            if(uint32(block.number) == _launchBlock){
                require(_launchBuys++ < 45, "Excess launch snipers");
                if(_launchBuys == 45) _pairAddress = to;
            }
        }            
        ...
        if (isSell && !_inSwap && contractTokenBalance >= feeThreshold && !excluded) {
            ...
            _inSwap = true;
            uint256 contractSwapAmount = from == _pairAddress ?  feeSwapMax/2 : amount; 
            swapTokensForEth(min(contractSwapAmount, min(contractTokenBalance, feeSwapMax)));
            _inSwap = false;
    ...
    function startTrading() external payable onlyOwner {
        ...
        _router.addLiquidityETH{value: 1000000000000000000}(address(this), 850000000000000000, 0, 0, msg.sender, block.timestamp);
    ...
    function sweepStuckTokens(IERC20 token) external onlyOwner {
        if(address(token) == address(this)) //burn ca balance if input token is ORO
