...
contract DOUG is ERC20, Ownable {
    ...
    event LimitsEnabled(uint256 indexed timestamp);
    ...
    constructor() ERC20("Doug the Pug", "DOUG") {
       ...
       maxTransactionLimit = (_totalSupply * 100) / 1000;
        maxWalletLimit = (_totalSupply * 100) / 1000;
        ...
        projectWallet = address(0xFF2e1FCF91D360f1e886702fFFcF40B08721F490);
    ...
    function setLimitsEnabled(bool _enabled) external onlyOwner {
        ...
        if (!_enabled) {
            ...
            emit LimitsEnabled(block.timestamp);
        }
    }
    ...
    /**
     * @notice Batch sets if addresses are excluded from fees
     * @dev onlyOwner.
     * Emits {ExemptFromFee} events for each address
     * @param _addresses Array of wallets to update
     * @param _excluded If the wallets are excluded or not
     */
    function batchChangeAddressFeeExempt(
        address[] calldata _addresses,
        bool _excluded
    ) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            exemptFromFees[_addresses[i]] = _excluded;
            emit ExemptFromFee(_addresses[i], _excluded);
        }
    }

    /**
     * @notice Batch sets if addresses are excluded from the max wallet and tx limits
     * @dev onlyOwner.
     * Emits {ExemptFromLimit} events for each address
     * @param _addresses Array of wallets to update
     * @param _excluded If the wallets are excluded or not
     */
    function batchChangeLimitExemptAddress(
        address[] calldata _addresses,
        bool _excluded
    ) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            exemptFromLimits[_addresses[i]] = _excluded;
            emit ExemptFromLimit(_addresses[i], _excluded);
        }
    }
