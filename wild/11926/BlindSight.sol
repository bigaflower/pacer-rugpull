 ...
 * https://BlindSightEth.VIP
 * https://t.me/BlindSightOnEth
 * https://x.com/BlindSightOnEth
 */
...
contract BlindSight is ERC20, Ownable {
    ...
    constructor() ERC20("BlindSight", "BLIND") {
        IDexRouter _dexRouter = IDexRouter(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        );

        antWhal_exemptAdd(address(_dexRouter), true);
        ...
        antWhal_exemptAdd(address(dexPair), true);
        ...
        marketingWallet = address(0x8CAB7B44b469eE67559DA9dc8deC64815f543C7c);

        // exclude from paying fees or having max transaction amount
        trxTax_exemptAdd(msg.sender, true);
        trxTax_exemptAdd(address(this), true);
        trxTax_exemptAdd(address(0xdead), true);
        trxTax_exemptAdd(marketingWallet, true);

        antWhal_exemptAdd(msg.sender, true);
        antWhal_exemptAdd(address(this), true);
        antWhal_exemptAdd(address(0xdead), true);
        antWhal_exemptAdd(marketingWallet, true);
     ...
     */
    function antWhal_disable() external onlyOwner {
        limitsEnabled = false;
        transferTaxTotal = 0;
     ...
     * @param newTxVal Base 1000, so 1% = 10
     */
    function antWhal_maxTx_Scng(
        uint256 newTxVal
    ) external onlyOwner {
        require(newTxVal >= 2, "Cannot set maxTx lower than 0.2%");
        maxTx = (newTxVal * totalSupply()) / 1000;
     ...
     * @param newWltVal Base 1000, so 1% = 10
     */
    function antWhal_walletLimit_new(
        uint256 newWltVal
    ) external onlyOwner {
        require(newWltVal >= 5, "Cannot set maxWallet lower than 0.5%");
        maxWallet = (newWltVal * totalSupply()) / 1000;
     ...
     */
    function trxTax_setwlt(address newMktWal) external onlyOwner {
