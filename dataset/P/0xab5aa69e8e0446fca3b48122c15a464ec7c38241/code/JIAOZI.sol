...
This token sets the "ShareFee wallet" to a random single holder wallet in the community (that has bought and is holding) for up to 60 mins at a time, thereafter it is changed randomely via the contract to another hodler. This will happen forever as long as it's traded. 

    Info:

    * I want to launch something that is fair 
    * So, I will launch without warning with small liquidity
    * No insiders, no bamboozles
    ...
    * This contract has a fee that will initially be set to 5% at launch and IS TO BE SHARED BETWEEN THE HOLDERS
    ...
    * Just HODL
    ...
    Jiaozi: Anyone who has bought and not sold is entered into the lottery to have their wallet assigned to the _ShareWallet and get the fees for a random 1 - 60 mins period, so buy and HODL
...
pragma solidity ^0.6.12;
        ...
        returns(uint256)
    {
        ...
        function _setShareWallet(address payable ShareWallet) external onlyOwner() {
         ...
         function StartTrading(bool _tradingEnabled) external onlyOwner() {
