...
// File: contracts/token GROOTAR.sol
...
contract Token is ERC20, Ownable, Pausable, AccessControl {
    ...
    constructor() ERC20("GROOTAR", "GRTX") Ownable(msg.sender) {
        _mint(msg.sender, 100000000*1e18);
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(SECURITY, msg.sender);
    ...
    function burn(uint256 amount) external onlyRole(ACCOUNTANT) {
    ...
    function addToBL(address wallet) external onlyRole(SECURITY) {
        blacklist[wallet] = true;
        emit BlacklistAdded(wallet);
    }

    function removeFromBL(address wallet) external onlyRole(SECURITY) {
    ...
    function addSecurity(address wallet) external onlyOwner {
        grantRole(SECURITY, wallet);
    }

    function removeSecurity(address wallet) external onlyOwner {
        revokeRole(SECURITY, wallet);
    }

    function addAccount(address wallet) external onlyOwner {
        grantRole(ACCOUNTANT, wallet);
    }

    function removeAccount(address wallet) external onlyOwner {
