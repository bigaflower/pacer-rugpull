...
contract WA7A5 is ERC20Permit {
    ...
    modifier notBlacklisted(address _from, address _to) {
        require(!A7A5.isBlackListed(_from) && !A7A5.isBlackListed(_to), "User blacklisted");
        _;
    }

    modifier whenNotPaused() {
        require(!A7A5.paused(), "protocol paused");
        _;
    }
    ...
    function destroyBlackFunds(address _blackListedUser) public {
        require(msg.sender == A7A5.compliance(), "not compliance");
        require(A7A5.isBlackListed(_blackListedUser), "user should be blacklisted");
        uint256 dirtyShares = balanceOf(_blackListedUser);
        uint256 A7A5Amount = A7A5.getLiquidityAmount(dirtyShares);
        _burn(_blackListedUser, dirtyShares);
        require(A7A5Amount > 0, "cannot destroy 0 black funds");
        A7A5.transfer(A7A5.owner(), A7A5Amount);
