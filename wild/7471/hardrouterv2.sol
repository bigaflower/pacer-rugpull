...
contract EtherVistaRouter is IEtherVistaRouter {
    ...
    modifier onlyTreasurySetter() {
        require(msg.sender == treasurySetter, 'EtherVistaRouter: FORBIDDEN');
        _;
    }
    ...
    function setTreasury(address payable _treasury, uint256 _treasuryFeeUsd) external onlyTreasurySetter {
        require(_treasury != address(0), 'EtherVistaRouter: ZERO_ADDRESS');
        treasury = _treasury;
        treasuryFeeUsd = _treasuryFeeUsd;
    }

    function setTreasurySetter(address _treasurySetter) external onlyTreasurySetter {
        require(_treasurySetter != address(0), 'EtherVistaRouter: ZERO_ADDRESS');
...
// **** SWAP (supporting fee-on-transfer tokens) ****
    ...
    {   
        ...
        treasury.transfer(usdcToEth(treasuryFeeUsd)); 
...
// **** LIBRARY FUNCTIONS ****
    ...
    function transferViaRouter(address token, address recipient, uint256 amount) external {
