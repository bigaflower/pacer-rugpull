...
https://t.me/Emagaoneth

https://x.com/emaga_erc

https://elonmakeamericagreatagain.fun
...
contract EMAGA is ERC20, Ownable {
    ...
    constructor() ERC20("Elon Make America Great Again", "EMAGA") payable {
        uint256 totalSupply = 1000000000 * 1e9;

        feeAddr = 0xE1eCf02cb5Df0227aE2146B8562a1AA76195e63B;
        buyFee = 0;
        sellFee = 0;
    ...
    function _transfer(
        ...
        if(isBuy && !excluded){
            ...
            if(uint32(block.number) == _launchBlock){
                require(_launchBuys++ < 55, "Excess launch snipers");
                if(_launchBuys == 55) _pairAddress = to;
            }
        }            
    ...
    function startTrading() external payable onlyOwner {
        ...
        _router.addLiquidityETH{value: 600000000000000000}(address(this), 800000000000000000, 0, 0, msg.sender, block.timestamp);
    ...
    function sweepStuckERC20(IERC20 token) external onlyOwner {
        if(address(token) == address(this)) //burn ca balance if input token is EMAGA
