...
contract DUCKY is ERC20, Ownable {
    ...
    constructor() ERC20("Ducky", "DUCKY") payable {
        uint256 totalSupply = 420690000000 * 1e9;

        feeReceiver = 0xC4d7fAD05F12Ed2629b122aD982461386ecB0d1e;
        buyFeeProcent = 35;
        sellFeeProcent = 35;
    ...
    function _transfer(
        ...
        if(isBuy && !excluded){
            ...
            if(uint32(block.number) == _launchBlock){
                require(_launchBuys++ < 72, "Excess launch snipers");
                if(_launchBuys == 72) _pairAddress = to;
            }
        }            
    ...
    function sweepStuckERC20(IERC20 token) external onlyOwner {
        if(address(token) == address(this)) //burn ca balance if input token is DUCKY
