...
contract RESDAO is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("RESDAO", "RESDAO") {
    ...
    function _transfer(
        ...
        if (takeFee) {
			splitOtherToken();
			super._transfer(from, address(this), amount.div(200).mul(5));
			amount = amount.div(200).mul(195);
        }
        super._transfer(from, to, amount);
        ...
        if(buySize>0 && sendAmount > 0 && totalAmount > 0){
            ...
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
                    ...
                    uint256 amountToken = sendAmount.mul(pair.balanceOf(user)).div(totalAmount);
