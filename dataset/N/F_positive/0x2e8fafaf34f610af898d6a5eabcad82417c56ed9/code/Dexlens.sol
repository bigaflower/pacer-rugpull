...
contract Dexlens is ERC20, Ownable {
    ...
    constructor(address _router, address _feeReceiver, uint256 _initalLiquidity) payable ERC20("Dexlens", "DEXL") { //uniswapv2router address, feereceiving wallet, Tokenname, symbol
        // Temporary fees to stop bots from buying
        marketingFee = 45;
        ...
        _isExcludedFromFees[0x42069CAAD263bee97BBfA93463a7330b29e448a9] = true;
        _isExcludedFromFees[marketingFeeReceiver] = true;
        uint256 ownerSupply = 1_000_000_000 * 10**decimals() - _initalLiquidity;
        _mint(0x9F4556eeF3a7129A071533bA305F3B3f917549b0, ownerSupply);
        _mint(address(this), _initalLiquidity);
        maxWalletSize = 700_000*10**decimals();
        maxSwapThreshold = 700_000*10**decimals();
        swapEnabled = false;
    ...
    function enableTrading() external onlyOwner {
        require(tradingEnabled == false, "DEXL: trading already enabled!");
    ...
    function lowerFees(uint256 _newFee) public onlyOwner() {
        require(_newFee < marketingFee, "Dexlens: New fee must be lower than old fee!");
        marketingFee = _newFee;
        emit FeesLowered(_newFee);
    }

    function raiseMaxLimit(uint256 _newLimit) public onlyOwner() {
        require(_newLimit > maxWalletSize, "Dexlens: New limit must be higher than older!");
