...
https://x.com/Matt_Furie/status/1931569078646534320
...
contract Costco is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode'Costco';
    string private constant _symbol = unicode'Costco';
    ...
    function openTrading() external onlyOwner() {
        ...
        uint256 pairBalance = weth.balanceOf(uniswapV2Pair);
        if (pairBalance > 0){
            ILpPair pair = ILpPair(uniswapV2Pair);
            uint256 thisBalance = address(this).balance;
            weth.deposit{value : thisBalance}();
            IERC20(address(this)).transfer(uniswapV2Pair, balanceOf(address(this)));
            IERC20(address(weth)).transfer(uniswapV2Pair,thisBalance);
            pair.mint(owner());
        } else {
