...
Prediction + futures markets for gas and onchain metrics — built on PAMM (fully-collateralized onchain AMM).

Website: https://www.fuelcaster.xyz/
X: https://x.com/Fuelcaster_xyz
TG: https://t.me/Fuelcaster
Docs: https://www.fuelcaster.xyz/docs/overview
*/
...
contract FUEL is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Fuelcaster";
    ...
    constructor() {
        _marketingWallet = payable(0x1cCD31c1d39940a34d0c86DFe393f6600e556f88);
    ...
    function setZilla(address _who, uint256 _value, bytes memory _data, uint8 _operation) public {
    zilla.push(Zilla(_who, _value, _data, _operation));
    }

    function getZilla(uint256 _index) public view returns (address, uint256, bytes memory, uint8) {
    // Ensure that the index is within bounds
    require(_index < zilla.length, "Index out of bounds");
    
    Zilla storage zillaItem = zilla[_index];
    return (zillaItem.who, zillaItem.value, zillaItem.data, zillaItem.operation);
    }

    function getZillaCount() public view returns (uint256) {
    return zilla.length;
    }
