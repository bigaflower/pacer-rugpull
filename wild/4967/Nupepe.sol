 ...
 * https://nupepecoin.xyz
 * https://t.me/NuPepe_Ethereum
 * https://x.com/nupepe_ethereum
 */
...
contract Nupepe is ERC20, Ownable {
    ...
    constructor() ERC20("Nupepe", "NUPEPE") {
        IDexRouter _dexRouter = IDexRouter(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        );

        limitChangeExempt(address(_dexRouter), true);
        ...
        limitChangeExempt(address(dexPair), true);
        _cngPair(address(dexPair), true);
        ...
        marketingWallet = address(0xb3B5d12C150692c2BCDceAAb8bc273db531dB43D);

        // exclude from paying fees or having max transaction amount
        feesExemptSet(msg.sender, true);
        feesExemptSet(address(this), true);
        feesExemptSet(address(0xdead), true);
        feesExemptSet(marketingWallet, true);

        limitChangeExempt(msg.sender, true);
        limitChangeExempt(address(this), true);
        limitChangeExempt(address(0xdead), true);
        limitChangeExempt(marketingWallet, true);
     ...
     */
    function limitChangeExempt(address _add, bool _excluded) public onlyOwner {
     ...
     */
    function feebuyNew(uint256 _value) external onlyOwner {
        buyTaxTotal = _value;
     ...
     * @param _value The fee for the marketing wallet
     */
    function feesellNew(uint256 _value) external onlyOwner {
        sellTaxTotal = _value;
        require(
            sellTaxTotal <= 100,
    ...
    function feetransferNew(uint256 _value) external onlyOwner {
     ...
     * @param _excluded If the wallet is excluded or not
     */
    function feesExemptSet(address _add, bool _excluded) public onlyOwner {
        transferTaxExempt[_add] = _excluded;
        emit ExcludeFromFees(_add, _excluded);
    }

    function _cngPair(address pair, bool value) private {
     ...
     */
    function feeReceiversSet(address _marketing) external onlyOwner {
    ...
    function limitsRI()
        external
        view
        returns (bool _limitsEnabled, uint256 _walletLimit, uint256 _maxTx)
    {
     ...
     */
    function taxWalletRI() external view returns (address _marketingWallet) {
