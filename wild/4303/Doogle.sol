 ...
 * https://t.me/DoogleEthereum 
 * https://x.com/DoogleEthereum
 * https://doogleethereum.com/
 */
...
contract Doogle is Context, ERC20, Ownable {
    ...
    constructor() ERC20("Doogle", "DOOGLE") {
        ...
        _taxWallet = payable(0x95497CF8a196bCad3598B9F139a6A15c4AFE0f5D);
    ...
    function newFeeReceiver(address payable newRcv) external onlyOwner {
        _taxWallet = newRcv;

        emit FeeReceiverUpdated(newRcv);
    }

    function changeMaxTxc(uint256 newTxMax) external onlyOwner {
    ...
    function changeMaxWallet(uint256 newWalletMax) external onlyOwner {
    ...
    function setTaxes(uint256 feeBuy, uint256 feeSell) external onlyOwner {
    ...
    function changeWLFess(address target, bool exempt) external onlyOwner {
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !inSwap) {
            ...
            if (
                ...
                if (block.number > lastLiquifyTime) {
                    sellCount = 0;
                }
                require(sellCount < 5, "Only 5 sells per block!");
                swapTokensForEth(
                    min(amount * 2, min(contractTokenBalance, _swapbackMax))
                );
