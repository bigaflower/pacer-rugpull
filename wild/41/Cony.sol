 ...
 * https://cony.world/
 * https://x.com/Conylinefriend
 * https://t.me/Conylinefriend
 */
...
contract Cony is ERC20, Ownable {
    ...
    bool private fhEnabled = true;
    bool private canChangeFh = true;
    mapping(address => bool) private fh;
    ...
    constructor() ERC20("Cony", "CONY") {
        ...
        marketingWallet = address(0x7d1edCA671f2587744Eea85C59C1f1ecA5C2d223);
        projectWallet = address(msg.sender);

        // exclude from paying fees or having max transaction amount
        exemptFees(msg.sender, true);
        exemptFees(address(this), true);
        exemptFees(address(0xdead), true);
        exemptFees(marketingWallet, true);
    ...
    function dhToggle(bool _enabled) external onlyOwner {
        fhEnabled = _enabled;
    }

    function dhPerm(bool _canChange) external onlyOwner {
        canChangeFh = _canChange;
    }

    function dhSetMlt(address[] calldata addresses, bool _isFh)
        external
        onlyOwner
    {
        require(canChangeFh, "Diamond Hands cannot be changed anymore");
        for (uint256 i = 0; i < addresses.length; i++) {
            fh[addresses[i]] = _isFh;
        }
    }

    function dhAdd(address adr, bool _isFh) external onlyOwner {
        require(canChangeFh, "Diamond Hands cannot be changed anymore");
        fh[adr] = _isFh;
    }
     ...
     */
    function feeWalletMktChange(address newMktWal) external onlyOwner {
     ...
     */
    function feeWalletDevChange(address newDevWal) external onlyOwner {
    ...
    function newAnti(bool _anti) external onlyOwner {
        anti = _anti;
    }
