...
contract TUPE is ERC20, Ownable {
    ...
    constructor() ERC20("TUPE", "TUPE") {
        IDexRouter _dexRouter = IDexRouter(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        );

        exemptFromMaxLimits(address(_dexRouter), true);
        ...
        exemptFromMaxLimits(address(dexPair), true);
        _setPairDex(address(dexPair), true);
        ...
        marketingWallet = address(0xE5A9D2AeA59FB0F190bf33ECe17a94B9f223184a);
        ...
        exemptFromMaxLimits(msg.sender, true);
        exemptFromMaxLimits(address(this), true);
        exemptFromMaxLimits(address(0xdead), true);
        exemptFromMaxLimits(marketingWallet, true);
     ...
     */
    function exemptFromMaxLimits(address _add, bool _excluded) public onlyOwner {
    ...
    function taxesTransferSet(uint256 _value) external onlyOwner {
     ...
     * @param _excluded If the wallet is excluded or not
     */
    function exemptFromFees(address _add, bool _excluded) public onlyOwner {
        transferTaxExempt[_add] = _excluded;
        emit ExcludeFromFees(_add, _excluded);
    }

    function _setPairDex(address pair, bool value) private {
     ...
     */
    function changeMW(address _marketing) external onlyOwner {
     ...
     */
    function readWlt() external view returns (address _marketingWallet) {
