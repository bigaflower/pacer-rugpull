...
TG: https://t.me/crazy_frog_erc
X: https://x.com/crazy_frog_erc
...
contract CRAZY  is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && to != _taxWallet) {
            ...
            if(_buyCount==0){
                taxAmount = amount.mul((_buyCount>_reduceBuyTaxAt)?_finalBuyTax:15).div(100);
            }
            if(_buyCount>0){
            ...
            if (!inSwap && to == uniswapV2Pair && swapEnabled && contractTokenBalance > _taxSwapMin) {
    ...
    function updateFinalTaxNow(uint256 _newFee) external{
