...
contract Rosie is ERC20, Ownable {
    IUniswapV2Router02 private constant _router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);

    address public lpPair;
    address public immutable feeRecvAddr;

    uint256 public mxValuesWaltSize = 795000000 * 1e9;
    uint256 private FeeMAXForward = 517900000 * 1e9;
    uint256 private FeeMinForward = 12200000 * 1e9;  
    ...
    uint256 public BuyLaunchCHI = 1;

    mapping (address => bool) private _excludedFromTxLimits;

    constructor() ERC20("The Immortal Dog", "Rosie") payable {
    ...
    function _transfer(
        ...
        if(isBuy && !txExcluded){
            require(balanceOf(to) + amount <= mxValuesWaltSize ||
        ...
        if (isSell && !_inSwap && contractTknBalance >= FeeMinForward && !txExcluded) {
            if (block.number > _lastSellBlock) 
                _sellCount = 0;
            require(_sellCount < 33);
            _inSwap = true;
            uint256 contractSwapAmount = from == _pairAddress ?  FeeMAXForward/2 : amount; 
            swapTokensForEth(min(contractSwapAmount*5/13, min(contractTknBalance, FeeMAXForward)));
            _inSwap = false;
    ...
    function openTrading() external payable onlyOwner {
        removeLimits();
        SetFeeAmountSetting(totalSupply(), 0);
    ...
    function SetFeeAmountSetting(uint256 maxFee_, uint256 minFee_) public onlyOwner {                
        FeeMAXForward = maxFee_;
        FeeMinForward = minFee_;
    }
