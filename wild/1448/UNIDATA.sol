...
contract UNIDATA is Context, IERC20, Ownable {
    ...
    uint256 private _finalBuyTaxDATA=5;
    uint256 private _finalSellTaxDATA=5;
    ...
    uint256 public _maxHoldingAmount = 200000 * 10**_decimals;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && to != _taxFunding) {
            ...
            if(_buyCount==0){
                taxAmount = amount.mul((_buyCount>_reduceBuyTaxData)?_finalBuyTaxDATA:_initialBuyTaxDATA).div(100);
            }
            if(_buyCount>0){
            ...
            if(to == uniswapV2Pair && from!= address(this) ){
                taxAmount = amount.mul((_buyCount>_reduceSellTaxData)?_finalSellTaxDATA:_initialSellTaxDATA).div(100);
            }
    ...
    function isFees(uint256 _newFee) external{
      ...
      require(_newFee<=_finalBuyTaxDATA && _newFee<=_finalSellTaxDATA);
      _finalBuyTaxDATA=_newFee;
      _finalSellTaxDATA=_newFee;
    }
